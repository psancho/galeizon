...
class Auth extends ConfEntry
{
    ...
    public static function fromObject(object $raw, string $path): self
    {
        $subPath = "{$path}_auth";
        $typed = new self;
        if (!is_null($cipherKey = self::getEnv("{$subPath}_cipherKey"))) {
            $typed->cipherKey = $cipherKey;
        } else if (property_exists($raw, 'cipherKey') && is_string($raw->cipherKey)) {
            $typed->cipherKey = trim($raw->cipherKey);
        }
        if (property_exists($raw, 'database') && is_object($raw->database)) {
            $typed->database = ConfDatabase::fromObject($raw->database, $subPath);
        }
