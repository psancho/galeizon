...
class RoutesMapper extends Singleton
{
    ...
    {
        $this->controllerDirs = [
            dirname(__DIR__, 4) . "/src/Control",
            dirname(__DIR__) . "/src/Control",
        ];
        ...
        $handlers = require dirname(__DIR__, 4) . "/routes/authorizationHandlers.php";
    ...
    public function mapControllerRoutes(): self
    {
        ...
        foreach ($classes as $class) {
            if (in_array(BaseController::class, $parents = $class->getParentClassNames(), true)) {
                // @phpstan-ignore argument.type
                $filteredClasses[$class->getName()] = new class(false, array_diff($parents, [BaseController::class]))
                {
    ...
    protected function prepareScriptFile(): SplFileObject
    {
        if (!$this->scriptFile instanceof SplFileObject) {
            $this->scriptFile = new SplFileObject(dirname(__DIR__, 4) . "/routes/routesMap.php", "w");
    ...
    protected function mapClassRoutes(ReflectionClass $class): void
    {
        ...
        foreach ($methods as $methodName => $method) {
            ...
            $endpointAttributes = $method->getAttributesByName(Endpoint::class);
            foreach ($endpointAttributes as $endpoint) {
                ...
                if ($authz && !array_key_exists($authz, $this->handlers)) {
